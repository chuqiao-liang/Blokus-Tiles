...
int main(int argc, char* argv[]) {
  vector<string> tileSet;
  string rows;
  int rowsTileSet{0}, N;  //  # of rows in a file not empty, textbox size
  if (argc == 1) {  //  if no arguments provided
    N = 5;  //  default textbox size=5
    while (getline(cin, rows)) {
      tileSet.push_back(rows);
      if (not rows.empty()) ++rowsTileSet;
    }
  } else {
    N = std::atoi(argv[1]);
    string blankline;
    if (argc == 2) {  //  no input file, use cin
      while (getline(cin, rows)) {
        tileSet.push_back(rows);
        if (not rows.empty()) ++rowsTileSet;
      }
      ...
      tileSet.push_back(blankline);
      input.close();
    }
  }
  if (not correctCharacters(tileSet)) return 1;
  if (not emptyLineAfterTile(tileSet)) return 2;
  if (not correctSize(tileSet, N)) return 2;
  if (emptyTile(tileSet, N)) return 3;
  //  put valid tileSet vector in an 2D array
  int rowsN = tileSet.size();
  char** ptileSetArr = new char*[rowsN];
  for (int c{0}; c < rowsN; ++c)
    ptileSetArr[c] = new char[N];

  for (int e{0}; e < rowsN; ++e) {
    for (int f{0}; f < N; ++f) {
      ptileSetArr[e][f] = tileSet[e][f];
    }
  }  // Checking uniqueness
  vector<int> emptyline;
  for (int a{0}; a < rowsN; ++a) {
    if (a % (N+1) == N)
      emptyline.push_back(a);
  }
  vector<int> countTile = numOfCells(ptileSetArr, rowsN, N);
  connectedTiles(ptileSetArr, rowsN, N, countTile, emptyline);

  for (int ref{0}; ref < (countTile.size()-1)*(N+1); ref = ref+N+1) {
    if (ptileSetArr[ref][0] == 'X')
      continue;
    int box = ref % N;
    for (int k{0}; k < countTile.size(); ++k) {
      if (k != box) {
        if (countTile[box] == countTile[k]) {
          for (int i{0}; i < N; ++i) {
            cycleLeft(ptileSetArr, ref, rowsN, N);
            sameTileSet(ptileSetArr, ref, rowsN, N);
          }
          for (int i{0}; i < N; ++i) {
            cycleUp(ptileSetArr, ref, rowsN, N);
            sameTileSet(ptileSetArr, ref, rowsN, N);
          }
          for (int deg{1}; deg < 5; ++deg) {
            rotate90Tile(ptileSetArr, ref, rowsN, N);
            sameTileSet(ptileSetArr, ref, rowsN, N);
            for (int i{0}; i < N; ++i) {
              cycleUp(ptileSetArr, ref, rowsN, N);
              sameTileSet(ptileSetArr, ref, rowsN, N);
              flipTileRow(ptileSetArr, ref, rowsN, N);
              sameTileSet(ptileSetArr, ref, rowsN, N);
              flipTileRow(ptileSetArr, ref, rowsN, N);
              flipTileCol(ptileSetArr, ref, rowsN, N);
              sameTileSet(ptileSetArr, ref, rowsN, N);
              flipTileCol(ptileSetArr, ref, rowsN, N);
            }
            for (int i{0}; i < N; ++i) {
  ...
  printArray(ptileSetArr, rowsN, N, argc, argv[3]);
  return 0;
}
